...
if( !empty($results) ) {
    if(!mysqli_query($con,"INSERT INTO laboratory(checkdate,pcrsamplerecievedate,cliniciandate,pcrsampleno,pcrsampletype,pcrsamplecolldate,vl_indication,artstartdate,drugregimen,pcrlabname,otherstatus,labregno,visitdate,resultstatus,artstatus,baselineorrepeat,tests,results,reportedby,reportdate,checkedby,pepid,additionaltests,clinicianname) 
VALUES('$checkdate','$pcrsamplerecievedate','$cliniciandate','$pcrsampleno','$pcrsampletype','$pcrsamplecolldate','$vl_indication','$artstartdate','$drugregimen','$pcrlabname','$otherstatus','$_POST[labregno]','$_POST[visitdate]','$resultstatus','$artstatus','$baselineorrepeat','$tests[$i]','$results[$i]','$reportedby','$reportdate','$checkedby','$_POST[pepid]','$_POST[additionaltests]','$_POST[clinicianname]')"))
...
{
	header("refresh:2;./patpharmviewph.php?pepid=".$pepid."&role=Lab");  
echo "Success: " . mysqli_affected_rows($con)." Row(s),Inserted";
...
//header('Location: ./patpedsummary.php?pepid='.$pepid);
	if ( mysqli_affected_rows($con) >= 1 )
{
	header("refresh:3;./patpharmviewph.php?pepid=".$pepid."&role=Lab");  
echo "Success: " . mysqli_affected_rows($con)." Row(s),Inserted";
echo "<p style='text-align: center;'><span style='font-size: large; font-family: georgia,palatino; color: #003366;'> Data Successfully Recorded to the Server </br> Continuing...</span></p> </br> ";
if(!mysqli_query($con,"insert into visit(visittype,visitdate,pepid) values('Laboratory Order/Result','$_POST[visitdate]','$_POST[pepid]')"));
}

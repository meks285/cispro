...
if(isset($_POST['tbdrug'])) {$tbdrug = implode("-", $_POST['tbdrug']);} else {$tbdrug = "";}
if(isset($_POST['oidrug'])) {$oidrug = implode("-", $_POST['oidrug']);} else {$oidrug = "";}
if(isset($_POST['oiqty'])) {$oiqty = implode("-", $_POST['oiqty']);} else {$oiqty = "";}
if(isset($_POST['oiduration'])) {$oiduration = implode("-", $_POST['oiduration']);} else {$oiduration = "";}
if(isset($_POST['tbqty'])) {$tbqty = implode("-", $_POST['tbqty']);} else {$tbqty = "";}
if(isset($_POST['tbduration'])) {$tbduration = implode("-", $_POST['tbduration']);} else {$tbduration = "";}

// Regimen Section
if(isset($_POST['drugname1'])) {$drugname1 = implode("/", $_POST['drugname1']);} else {$drugname1 = "";}
if(isset($_POST['drugdose1'])) {$drugdose1 = implode("/", $_POST['drugdose1']);} else {$drugdose1 = "";}
if(isset($_POST['frequency'])) {$frequency = implode("-", $_POST['frequency']);} else {$frequency = "";}

$_SESSION=$_POST;
$pepid = $_POST['pepid'];
// =============== code for Submit_input type ================================

$query1="SELECT * from pharmacy where pepid='$_POST[pepid]' and visitdate='$_POST[visitdate]'";
			$result = mysqli_query($con,$query1);
	if (mysqli_num_rows($result)==0){
    if(!mysqli_query($con,"INSERT INTO pharmacy (quantitydisp,quantitypres,pepid, visitdate, patienttype,visittype,frequency, pregyn,refillyn,subswitch,tbdrug,tbqty,tbduration,oidrug,oiqty,oiduration,dispensedate,
otherdrugs,otherquantity,otherduration,orderedby,orderdate,dispensedby,pharmdispensedate,pickedupby,pickupdate,counseledby,counseldate,errordrugname,errornature,errorconsequence,errorrepdate, drugname1,drugdose1,regduration1,otherdrugs1,otherquantity1,otherduration1,otherdrugs2,otherquantity2,otherduration2,weight,height,pharmregno,nextapptdate)
VALUES
('$_POST[quantitydisp]','$_POST[quantitypres]','$_POST[pepid]','$_POST[visitdate]','$_POST[patienttype]','$_POST[visittype]','$frequency','$pregyn','$refillyn',
'$subswitch','$tbdrug',
'$tbqty','$tbduration','$oidrug','$oiqty','$oiduration','$_POST[pharmdispensedate]',
...
'$_POST[pickupdate]','$_POST[counseledby]','$_POST[counseldate]','$_POST[errordrugname]','$_POST[errornature]','$_POST[errorconsequence]','$_POST[errorrepdate]','$drugname1','$drugdose1','$_POST[regduration1]','$_POST[otherdrugs1]','$_POST[otherquantity1]','$_POST[otherduration1]','$_POST[otherdrugs2]','$_POST[otherquantity2]','$_POST[otherduration2]','$_POST[weight]','$_POST[height]','$_POST[pharmregno]','$_POST[nextapptdate]')"))
...
$sql="INSERT into AUDIT_TRAIL (server_name,user_name,action) VALUES('$_SERVER[REMOTE_ADDR]','$_SESSION[login_id]','NEW - Pharmacy.: $_POST[pepid]')";
$result=mysqli_query($con,$sql);
}
	header("refresh:3;./patsummary.php?pepid=".$pepid."&login_id=".$_SESSION['login_id']);  
echo "Success: " . mysqli_affected_rows($con)." infondr(s),Inserted";
...
echo "Failure: " . mysqli_affected_rows($con)." infondr(s),Inserted";

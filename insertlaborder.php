...
if(isset($_POST['resultstatus'])) {$resultstatus=$_POST['resultstatus'];}else {$resultstatus="";}
  //=======Controlling ART STATUS for VL Form and Laboratory Form ======
if(isset($_GET['status'])=='VL') {
	$artstatus='ART';
}
elseif(isset($_POST['artstatus'])) {
...
if(isset($_POST['pcrlabname'])) {$pcrlabname=$_POST['pcrlabname'];}else {$pcrlabname="";}
// New Addition from 2016 GON Forms
if(isset($_POST['otherstatus'])) {$otherstatus = implode(", ", $_POST['otherstatus']);} else {$otherstatus = "";}
if(isset($_POST['cliniciandate'])) {$cliniciandate=$_POST['cliniciandate'];}else {$cliniciandate="";}
if(isset($_POST['pcrlabname'])) {$pcrlabname=$_POST['pcrlabname'];}else {$pcrlabname="";}
if(isset($_POST['pcrsampleno'])) {$pcrsampleno=$_POST['pcrsampleno'];}else {$pcrsampleno="";}
if(isset($_POST['pcrsampletype'])) {$pcrsampletype=$_POST['pcrsampletype'];}else {$pcrsampletype="";}
if(isset($_POST['pcrsamplecolldate'])) {$pcrsamplecolldate=$_POST['pcrsamplecolldate'];}else {$pcrsamplecolldate="";}
if(isset($_POST['pcrsamplerecievedate'])) {$pcrsamplerecievedate=$_POST['pcrsamplerecievedate'];}else {$pcrsamplerecievedate="";}
if(isset($_POST['vl_indication'])) {$vl_indication=$_POST['vl_indication'];}else {$vl_indication="";}
if(isset($_POST['artstartdate'])) {$artstartdate=$_POST['artstartdate'];}else {$artstartdate="";}
if(isset($_POST['drugregimen'])) {$drugregimen=$_POST['drugregimen'];}else {$drugregimen="";}
if(isset($_POST['checkdate'])) {$checkdate=$_POST['checkdate'];}else {$checkdate="";}
//For Viral Load
...
if(isset($_POST['collectedby'])) {$collectedby=$_POST['collectedby'];}else {$collectedby="";}
if(isset($_POST['collectdate'])) {$collectdate=$_POST['collectdate'];}else {$collectdate="";}
if(isset($_POST['resultintepretation'])) {$resultintepretation=$_POST['resultintepretation'];}else {$resultintepretation="";}
if(isset($_POST['assayedby'])) {$assayedby=$_POST['assayedby'];}else {$assayedby="";}
if(isset($_POST['assaydate'])) {$assaydate=$_POST['assaydate'];}else {$assaydate="";}
if(isset($_POST['approvedby'])) {$approvedby=$_POST['approvedby'];}else {$approvedby="";}
if(isset($_POST['approvedate'])) {$approvedate=$_POST['approvedate'];}else {$approvedate="";}
// End of New Addition
$pepid = $_POST['pepid'];
...
if( !empty($results) ) {
    if(!mysqli_query($con,"INSERT INTO laboratory(approvedate,approvedby,assaydate,assayedby,resultintepretation,collectdate,collectedby,orderdate,orderedby,checkdate,pcrsamplerecievedate,cliniciandate,pcrsampleno,pcrsampletype,pcrsamplecolldate,vl_indication,artstartdate,drugregimen,pcrlabname,otherstatus,labregno,visitdate,resultstatus,artstatus,baselineorrepeat,tests,results,reportedby,reportdate,checkedby,pepid,additionaltests,clinicianname) 
VALUES('$approvedate','$approvedby','$assaydate','$assayedby','$resultintepretation','$collectdate','$collectedby','$orderdate','$orderedby','$checkdate','$pcrsamplerecievedate','$cliniciandate','$pcrsampleno','$pcrsampletype','$pcrsamplecolldate','$vl_indication','$artstartdate','$drugregimen','$pcrlabname','$otherstatus','$_POST[labregno]','$_POST[visitdate]','$resultstatus','$artstatus','$baselineorrepeat','$tests[$i]','$results[$i]','$reportedby','$reportdate','$checkedby','$_POST[pepid]','$_POST[additionaltests]','$clinicianname')"))
...
$sql="INSERT into AUDIT_TRAIL (server_name,user_name,action) VALUES('$_SERVER[REMOTE_ADDR]','$_SESSION[login_id]','NEW - Laboratory.: $_POST[pepid]')";
$result=mysqli_query($con,$sql);
}
	header("refresh:2;./patsummary.php?pepid=".$pepid."&login_id=".$_SESSION['login_id']);  
echo "Success: " . mysqli_affected_rows($con)." Row(s),Inserted";
...
if(!mysqli_query($con,"insert into visit(visittype,visitdate,pepid) values('Laboratory Order/Result','$_POST[visitdate]','$_POST[pepid]')"));
}
else {
	$link_address = 'javascript:history.go(-1)';
echo "Failure: " . mysqli_affected_rows($con)." Row(s),Inserted";
echo "</br>";
echo "Database error Output: " . mysqli_error($con)."";
...
$result=mysqli_query($con,$sql);
	...
	header("refresh:3;./patpedsummary.php?pepid=".$pepid."&login_id=".$_SESSION['login_id']);  
echo "Success: " . mysqli_affected_rows($con)." Row(s),Inserted";
echo "<p style='text-align: center;'><span style='font-size: large; font-family: georgia,palatino; color: #003366;'> Data Successfully Recorded to the Server </br> Continuing...</span></p> </br> ";
if(!mysqli_query($con,"insert into visit(visittype,visitdate,pepid) values('Laboratory Order/Result','$_POST[visitdate]','$_POST[pepid]')"));
}
...
echo "<p style='text-align: center;'><span style='font-size: large; font-family: georgia,palatino; color: RED;'> Data could not be saved </br> Please try again </br> <a href='".$link_address."'>BACK</a> </span></p> </br> ";
	...
	$link_address = 'javascript:history.go(-1)';
//echo "Failure: " . mysqli_affected_rows($con)." Row(s),Inserted";
echo "<p style='text-align: center;'><span style='font-size: large; font-family: georgia,palatino; color: RED;'> Data was not saved </br> This VISITDATE: ".$_POST['visitdate']." entry has been made already </br> <a href='".$link_address."'>BACK</a> </span></p> </br> ";
	}

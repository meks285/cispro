...
$query = "SELECT patient.surname,patient.othernames,pharmacy.pharmacyid,pharmacy.pepid, DATE_FORMAT(pharmacy.visitdate, '%d-%b-%Y') visitdate,
 ...
 pharmacy.duration,pharmacy.regimen,pharmacy.quantitydisp,pharmacy.nextapptdate as nextapptdate,DATE_FORMAT(pharmacy.dispensedate, '%d-%b-%Y') dispensedate, drugname1 as regimend 
...
$result = mysqli_query($con,"SELECT patient.surname,patient.othernames,pharmacy.pharmacyid,pharmacy.pepid,pharmacy.visitdate,
 pharmacy.patienttype,pharmacy.drugno1,pharmacy.visittype,pharmacy.tbdrug,pharmacy.oidrug,pharmacy.otherdrugs,pharmacy.drugdose1, pharmacy.pregyn,pharmacy.refillyn,pharmacy.subswitch,pharmacy.drugname,pharmacy.strength,pharmacy.frequency,
 pharmacy.duration,pharmacy.regimen,pharmacy.quantitydisp,DATE_FORMAT(pharmacy.nextapptdate, '%d-%b-%Y') as nextapptdate,DATE_FORMAT(pharmacy.pharmdispensedate, '%d-%b-%Y') dispensedate, CONCAT(drugname1,'/',COALESCE(drugname2,''),'/',COALESCE(drugname3,'')) as regimend 
FROM pharmacy JOIN patient
ON patient.pepid=pharmacy.pepid
and pharmacy.pepid='$_SESSION[pepid]'
order by pharmacy.pharmacyid");
...
while($row = mysqli_fetch_array($result))
  ...
  echo "<tr>";
  echo '<td><a style="color:blue" href="patpharmbrowse.php?visitdate=' . $row['visitdate'] . '&amp;pepid=' . $row['pepid'] . '&amp;username=' . $_GET['username'] . '">Browse</a></td>';
  echo "<td>" . $row['pepid'] . "</td>";
  echo "<td>" . $row['visitdate'] . "</td>";
  echo "<td>" . $row['visittype'] . "</td>";
  echo "<td>" . $row['patienttype'] . "</td>";
  echo "<td>" . $row['regimend'] . "</td>";
  echo "<td>" . $row['drugdose1'] . "</td>";
  echo "<td>" . $row['drugno1'] . "</td>";

...
include './mandate_/lidh.php';
$nokaddr=mysqli_real_escape_string($con,$nokaddr);
} 
 else {
$nokaddr = "";
}
...
include './mandate_/lidh.php';
$addrlga=mysqli_real_escape_string($con,$addrlga);
} 
 else {
$addrlga = "";
}
...
include './mandate_/lidh.php';
$addrwardvillage=mysqli_real_escape_string($con,$addrwardvillage);
} 
 else {
$addrwardvillage = "";
}
...
include './mandate_/lidh.php';
$nokwardvillage=mysqli_real_escape_string($con,$nokwardvillage);
} 
 else {
$nokwardvillage = "";
}

if(isset($_POST['lga'])){ $lga = $_POST['lga']; include './mandate_/lidh.php';
 $lga=mysqli_real_escape_string($con,$lga);}  else {$lga = "";}
if(isset($_POST['surname'])){ $surname = $_POST['surname']; include './mandate_/lidh.php';
 $surname=mysqli_real_escape_string($con,$surname);}  else {$surname = "";}
if(isset($_POST['othernames'])){ $othernames = $_POST['othernames']; include './mandate_/lidh.php';
 $othernames=mysqli_real_escape_string($con,$othernames);}  else {$othernames = "";}
...
wardvillage,town,addrlga,phoneno,nokname,nokaddr,nokwardvillage,noktown,nokstate,nokrelationship,nokphoneno,enroldate,hivposdate,pprevarvdrugs)
VALUES
('$pepid','$_POST[uniqueid]','$_POST[hospitalno]','$surname','$_POST[state]','$othernames','$_POST[sex]',
'$lga','$_POST[facilityname]',
...
echo mysqli_error($con);
	$querymigration = "SELECT pepid FROM migration_data WHERE pepid = '$_POST[pepid]'";
	$resultmigration = mysqli_query($con,$querymigration);
    if (mysqli_num_rows($resultmigration)==1){
		//header("refresh:3;./pedinitialeval.php?username=".$_POST['username']);
	}
	else{
header("refresh:3;./newpatientiniteval.php?username=".$_POST['username']."&pepid=".$_POST['pepid']);
	}
...
include './mandate_/lidh.php';
	...
	$resultmigration = mysqli_query($con,$querymigration);
    if (mysqli_num_rows($resultmigration)==1){
	$link_address = './msf/migration_radet/index.php';
	echo "<p style='text-align: center;'><span style='font-size: large; font-family: georgia,palatino; color: GREEN;'>Going Forward</br> <a href='".$link_address."'>CONTINUE MIGRATION</a> </span></p> </br> ";
		echo ".<br>";
		echo ".<br>";
		echo "MIGRATION SUCCESSFUL...";
        echo ".<br>";
		echo ".<br>";
		echo "DELETING MIGRATED PATIENT DATA...";
        $prevQuery = "delete from migration_data WHERE pepid='$_POST[pepid]'";
        $prevResult = $con->query($prevQuery);
		echo ".<br>";
		echo "DONE...";
		echo ".<br>";		
//    if(!mysqli_query($con,"DELETE from pharmacy where pepid='$_POST[pepid]'"))
	...
	$resultmigration = mysqli_query($con,$querymigration);
    if (mysqli_num_rows($resultmigration)==1){
	$querymigsub = "SELECT v_load FROM `migration_data` WHERE pepid='$_POST[pepid]' and (v_load is not null or v_load!='')";
	$resultquerymigsub = mysqli_query($con,$querymigsub);
    if (mysqli_num_rows($resultquerymigsub)==1){
	$query4="insert into laboratory(pepid,visitdate,tests,results) values('$_POST[pepid]','$_POST[lab_date]','Viral Load','$_POST[v_load]')";
	$result4 = mysqli_query($con,$query4);

	echo mysqli_error($con);
	echo ".<br>";
	echo ".<br>";
	echo "Importing Viral Load-Lab Data From Migration Table...";
	echo "GOING FORWARD...";
		}
		
	$querymigsub_pharm = "SELECT ARTStartDate,LastVisitDate FROM `migration_data` WHERE pepid='$_POST[pepid]'";
	$resultquerymigsub_pharm = mysqli_query($con,$querymigsub_pharm);
	$row = $resultquerymigsub_pharm->fetch_assoc();
	
	    if (mysqli_num_rows($resultquerymigsub_pharm)==1){
	$query4="insert into pharmacy(pepid,visitdate,drugname1,patienttype,visittype,pharmdispensedate) values('$_POST[pepid]','$_POST[ARTStartDate]','$_POST[RegimenatARTStart]','ART','initial','$_POST[ARTStartDate]')";
	$result4 = mysqli_query($con,$query4);
	
		echo mysqli_error($con);
	echo ".<br>";
	echo ".<br>";
	echo "Importing Initial-Pharmacy Data From Migration Table...";
	echo "GOING FORWARD...";

		if($row['ARTStartDate']=$row['LastVisitDate']){
			exit();
		}else{
	$query4="insert into pharmacy(pepid,visitdate,drugname1,patienttype,visittype,pharmdispensedate) values('$_POST[pepid]','$_POST[LastVisitDate]','$_POST[CurrentRegimen]','ART','Follow-Up','$_POST[LastVisitDate]')";
	$result4 = mysqli_query($con,$query4);

	echo mysqli_error($con);
	echo ".<br>";
	echo ".<br>";
	echo "Importing Follow-Up Pharmacy Data From Migration Table...";
	echo "GOING FORWARD...";
	echo ".<br>";
	echo ".<br>";
	}

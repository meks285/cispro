...
<form id="formId" name="form" method="POST" action="carecard2insert.php">
<fieldset>
...
$('#submit').click( function() {
	...
	else if($('#nextapptdate').val() == ''){
		alert('Next appointment date value missing');
	}
	else if($('#clinician').val() == ''){
		alert('Clinician name missing');
	}
	else{
			var query = $('#formId').serialize();
			var pepid = $('#pepid').val();
        	var url = 'carecard2insert.php';
        	$.post(url, 
        	query, 
        	function (response) {
        	alert ("Record Added");

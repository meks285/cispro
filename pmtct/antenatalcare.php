...
if(mysql_num_rows(mysql_query("SHOW COLUMNS FROM `exposed_infant` LIKE '".$column13."'"))==0){
	...
	// Addition of Multiple New Columns
if(mysql_num_rows(mysql_query("SHOW COLUMNS FROM `antenatal_care` LIKE '".$column14."'"))==0){
	$results_C1 = mysql_query("ALTER TABLE antenatal_care
	ADD COLUMN gest_age_art varchar(100) DEFAULT NULL;");
	}
	}
  ...
  `arv_start_date` date DEFAULT NULL,
  `gest_age_art` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`id`)
)");
...
var gest_age_art = _("gest_age_art").value;
...
var ajax = ajaxObj("POST", "antenatalcare.php");
...
function getbmi(){
	...
	var bmi;
	var heightsquare;
	heightsquare = parseFloat(height) * parseFloat(height);
	bmi = parseInt(weight)/parseFloat(heightsquare);
...
<input type="text" name="phoneno" id="dateofbirth" value="<?php if(isset($info2['phoneno'])){echo $info2['phoneno'];}else{echo "";} ?>" style="width:120px;height:10px;"  required> 
    </fieldset>
         </div>
        <div style="display: table-cell;"> 
        <fieldset style="height:20px; border:none">
...
<tr><td style="width:20px;">1</td>
<td><b>EDD</b></td><td><input type="date" onfocus="emptyElement('status','status3')" name="months_since_last_menstrual_period" id="months_since_last_menstrual_period"  style="width:120px;" required>(Auto)</td>
</tr>
<tr><td>2</td>
...
<h3 style="text-align:center">ART INITIATION/MONITORING</h3>
...
<tr><td style="width:20px;">7a</td>
<td>Gestational Age at commencement of ART (If Starting ART)</td>
<td>
Age
<input type="text" name="gest_age_art" onfocus="emptyElement('status','status3')" id="gest_age_art"  style="width:120px;"/>
</td>
</tr>
...
<button type="button" id="submitbtn" name="submitbtn" style="float: right !important; cursor:pointer;" onClick="saveantenatalcare()" class="btn btn-success">Submit</button>
</fieldset>

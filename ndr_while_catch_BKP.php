...
WHEN '2/52' THEN 0.5
WHEN '1/52' THEN 0.25
ELSE artduration END as DurationOnArt,ROUND(weight) as Weight,bp as BloodPressure,CASE fp 
WHEN 'Condoms' THEN 'FP1' 
WHEN 'Oral contraceptive pills' THEN 'FP2' 
WHEN 'Injectable/Implantable hormones' THEN 'FP3' 
WHEN 'Diaphragm/cervical caps' THEN 'FP4' 
...
ELSE '' END as CodeDescTxtCTX,cd4value as cd4value,visitdate as CD4TestDate,nextapptdate as NextAppointmentDate
from clinicaleval
where pepid='$row[pepid]' and visitdate !='0000-00-00' group by visitdate order by visitdate ASC";
...
ELSE '' END as CodeDescTxt,results as Value1,clinicianname as Clinician,reportedby as ReportedBy,CheckedBy as checkedby    
from laboratory                                                                                                                        
...
WHEN 'WBC' THEN '81' 
WHEN 'WBC Diff' THEN '82'
ELSE '' END)!='' order by visitdate ASC";
		//$resulta=mysqli_query($con,$sqla);
		//$info1a=mysqli_fetch_assoc($resulta);
		//============
...
order by visitdate DESC LIMIT 1";
	...
	//$Regimen->addChild("SwitchIndicator", "false");
		...
		include './mandate_/lidh2.php';
		$sql="SELECT dispensedate as DateRegimenStarted,DATE_FORMAT(dispensedate,'%d') as DateRegimenStartedDD,DATE_FORMAT(dispensedate,'%c') as DateRegimenStartedMM,DATE_FORMAT(dispensedate,'%Y') as DateRegimenStartedYYYY FROM pharmacy where pepid='$row[pepid]' and lower(oidrug) like '%ctx%' and visitdate !='0000-00-00' and pharmdispensedate !='0000-00-00' order by visitdate ASC LIMIT 1";
		$result=mysqli_query($con,$sql);
		$info1=mysqli_fetch_assoc($result);

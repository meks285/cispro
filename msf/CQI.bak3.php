...
ob_end_clean();
	...
	if ($indicator=='ART 2'){
		echo 'Executing ART 2... Pls Wait.';	
			...
			//ART 2
$art2NUM="select distinct ph.pepid
...
$art2DEN="select distinct ph.pepid
from pharmacy ph join (SELECT labid, pepid, visitdate,results
                      from laboratory
                      WHERE tests='CD4') la
on (ph.pepid=la.pepid)
...
$art3NUM="SELECT distinct t2.pepid
...
$art3DEN="SELECT distinct t2.pepid
FROM patient p JOIN pharmacy t2 
ON (p.pepid=t2.pepid)
where lower(t2.patienttype)='art' 
and lower(t2.visittype)='initial'
...
$cc3NUM="SELECT distinct l1.pepid
...
$cc3DEN="SELECT distinct l1.pepid
FROM laboratory l1 JOIN patient p
ON(p.pepid=l1.pepid)
WHERE p.pepid = (SELECT distinct l1.pepid
		FROM clinicaleval l1
...
// LAB 4
$lab4NUM="SELECT distinct l1.pepid
FROM laboratory l1
JOIN patient p
ON(p.pepid=l1.pepid)
...
ob_end_clean();	
	...
	if ($indicator=='LAB 5'){
ob_start();
		echo 'Executing LAB 5... Pls Wait.';	

// LAB 5
$lab5NUM="SELECT distinct p.pepid
...
$resultlab5NUM=mysql_query($lab5NUM);
$cntcclab5NUM=mysql_num_rows($resultlab5NUM);

$lab5DEN="SELECT distinct p.pepid
FROM pharmacy p1 JOIN patient p
ON (p.pepid=p1.pepid)
where p1.pepid = (select distinct l1.pepid
		  from laboratory l1
...
AND lower(patienttype)='art' and lower(visittype)='initial';";
$resultlab5DEN=mysql_query($lab5DEN);
$cntcclab5DEN=mysql_num_rows($resultlab5DEN);

ob_end_clean();	
echo 'CQI Reports Generated... Thank You.';
	}
